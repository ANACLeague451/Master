...
public class MyAgent extends DefaultParty {
    ...
    private void myTurn() throws IOException {
        ...
        if (currentRound < kValue * 2) {
            for (Bid p : this.bidsUtilityMap.keySet())
                opponentUtilities.add(0.5);
        } else if (currentRound % kValue == 0) {
            for (Bid p : this.bidsUtilityMap.keySet()) {
                // update opponent utilities
            }
        }
...
//        System.out.println("BOI" + currentRound);
    ...
    private void updateAcceptable(Bid nextBid) {
        ...
        this.acceptableUtilityValue = bidUtil;
//        System.out.println("New acct: " + acceptableUtilityValue + " for util " + bidUtil);
    }
    ...
    private void issueWeightEstimation() {
        ...
        for (Object[] val : chivalues)
            System.out.println("CHI v " + val[0] + " " + val[1]);
        this.updateIssueWeights(chivalues);
    }
...
//        getReporter().log(Level.INFO, "------CURRENT ROUND------" + currentRound);
    ...
    private void updateIssueWeights(ArrayList<Object[]> chivalues) {
        int numIssues = chivalues.size();
        double avg = 0.0;
        ...
        if (progress instanceof ProgressRounds) {
            currentRound = ((ProgressRounds) progress).getCurrentRound();
            totalRound = ((ProgressRounds) progress).getTotalRounds();
        }

        // find change after each window
        double avgChange = 1 / (double) numIssues / ((double) totalRound / kValue - 1);
        double change = (1.4 - ((double) currentRound / totalRound)) * avgChange;

        for (int i = 0; i < numIssues; ++i)
            System.out.println("old IW " + this.issueWeights.get(i)[1].toString());

        // update each issue weight
        for (int i = 0; i < numIssues; ++i) {
            double dist = (double) chivalues.get(i)[1] - avg;
            this.issueWeights.set(i, new Object[]{this.issueWeights.get(i)[0],
                    (double) this.issueWeights.get(i)[1] - dist * change});
        }

        for (int i = 0; i < numIssues; ++i)
            System.out.println("new IW " + this.issueWeights.get(i)[1].toString());
    }
